...
// telechain: if pwallet != NULL it will attempt to create coinstake
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, CWallet* pwallet, bool* pfPoSCancel)
{
    ...
    if (pwallet)  // attemp to find a coinstake
    {
        *pfPoSCancel = true;
        ...
        if (*pfPoSCancel)
            return nullptr; // telechain: there is no point to continue if we failed to create coinstake
    }
    else
    ...
    if (!TestBlockValidity(state, chainparams, *pblock, pindexPrev, false, false, false)) {   // telechain: we do not check block signature here, since we did not sign it yet
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TelechainMiner: generated block is stale");
    }
...
void PoSMiner(CWallet *pwallet)
{
    ...
    RenameThread("telechain-stake-minter");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                ...
                LogPrintf("Error in TelechainMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
    ...
    {
        LogPrintf("TelechainMiner terminated\n");
    return;
        // throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("TelechainMiner runtime error: %s\n", e.what());
        return;
    }
}
...
void static TelechainMiner(const CChainParams& chainparams)
{
    LogPrintf("TelechainMiner started\n");
    RenameThread("telechain-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in TelechainMiner: Keypool ran out, please call keypoolrefill before restarting the mining hread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running TelechainMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateTelechains(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TelechainMiner, boost::cref(chainparams)));
}

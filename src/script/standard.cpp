...
static bool SolverInner(const CScript& scriptPubKey, txnouttype& typeRet, vector<vector<unsigned char> >& vSolutionsRet)
{
    ...
    if (scriptPubKey.IsPayToScriptHash(0))  // telechain: there is no need to check for names here, because caller of this function will do just that
    {
    ...
    if (scriptPubKey.IsWitnessProgram(witnessversion, witnessprogram, 0)) {  // telechain: there is no need to check for names here, because caller of this function will do just that
...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, vector<vector<unsigned char> >& vSolutionsRet)
{
    ...
    // telechain: remove name (if any exist) and try again
...
// telechain: scriptPubKey should come from vout[0]

...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help") || IsArgSet("-version")) {
        ...
        if (!IsArgSet("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  telechain-cli [options] <command> [params]  " + strprintf(_("Send command to %s"), _(PACKAGE_NAME)) + "\n" +
                  "  telechain-cli [options] -named <command> [name=value] ... " + strprintf(_("Send command to %s (with named arguments)"), _(PACKAGE_NAME)) + "\n" +
                  "  telechain-cli [options] help                " + _("List commands") + "\n" +
                  "  telechain-cli [options] help <command>      " + _("Get help for a command") + "\n";
...
UniValue CallRPC(const std::string& strMethod, const UniValue& params)
{
        ...
        if (!GetAuthCookie(&strRPCUserColonPass)) {
            throw std::runtime_error(strprintf(
                _("Could not locate RPC credentials. No authentication cookie could be found, and no rpcpassword is set in the configuration file (%s)"),
                    GetConfigFile(GetArg("-conf", TELECHAIN_CONF_FILENAME)).string().c_str()));
    ...
    // telechain:
    // we get telechaind output (possibly binary) of .write() that either:

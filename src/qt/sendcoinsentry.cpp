    ...
    platformStyle(_platformStyle)
{
    ...
    ui->payTo->setValidator(0);  // telechain: disable validator so that we can type names
    ui->payAmountExch->setValidator( new QDoubleValidator(0, 1e20, 8, this) );
    qsExchInfo = "<html><head/><body><p><span style=\" font-weight:600;\">"+
            tr("WARNING: You're using external service! Telechain is not responsible for functionality and correct behavior of this service.")+"</span><br/>"+
...
void SendCoinsEntry::setModel(WalletModel *_model)
{
    ...
    // telechain: initialize exchange box
...
//    }
    ...
    // telechain: disabled in this version
...
void SendCoinsEntry::on_requestPaymentButton_clicked()
{
    ...
    multimap<double, pair<Exch *, bool> > mapExch;   // double: tlc per 1 btc
    ...
    for (Exch* exch : eBox.m_v_exch)
    {
        ...
        mapExch.insert(pair<double, pair<Exch *, bool> >(exch->EstimatedTLC(dPay), pair<Exch *, bool>(exch, valid)));
    }
    ...
    for (const auto& p : mapExch)
    {
        ...
        if (valid)
            qsEntry = QString::number(p.first)+"tlc ["+QString::fromStdString(p.second.first->Name())+"]";
        else

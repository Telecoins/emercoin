...
over /Applications/Telechain-Qt (on Mac) or telechaind/telechain-qt (on Linux).
...
Using the standard "./autogen.sh; ./configure; make" to build Telechain-Qt and
telechaind makes it easier for experienced open source developers to contribute
to the project.
...
Another change in the 0.9 release is moving away from the telechaind executable
functioning both as a server and as a RPC client. The RPC client functionality
("tell the running bitcoin daemon to do THIS") was split into a separate
executable, 'telecli'. The RPC client code will eventually be removed from
telechaind, but will be kept for backwards compatibility for a release or two.
...
- Add '-disablewallet' mode to let telechaind run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with telechaind
...
- OS X: Fix telechain-qt startup crash when clicking dock icon

...
For example:
    ...
    $ src/telecli -stdin walletpassphrase
    mysecretcode
    120
...
The two phases are distinct in the Telechain-Qt GUI. During the first one,
...
- #7550 `8b958ab` Input-from-stdin mode for telecli (laanwj)
...
- #7613 `3798e5d` Add autocomplete to telechain-qt's console window (GamerSg)
...
- #8256 `af2421c` BUG: telechain-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from telechaind (sipa)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for telechaind procs (MarcoFalke)
...
- #8164 `0f24eaf` [TELECHAIN-TX] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of telechaind startup (laanwj)

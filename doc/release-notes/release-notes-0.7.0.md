...
We recommend that everybody running prior versions of telechaind/Telechain-Qt
...
in your terminal, then install the telechain-qt package:
  sudo apt-get install telechain-qt
...
Code:
/Applications/Telechain-Qt
(on Mac) or
Code:
telechaind/telechain-qt
(on Linux).
...
* (Windows only): enable ASLR and DEP for telechain-qt.exe
* (Windows only): add meta-data to telechain-qt.exe (e.g. description)
...
* Bash programmable completion for telechaind(1)

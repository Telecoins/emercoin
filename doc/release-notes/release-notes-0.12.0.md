...
This allows running telechaind without having to do any manual configuration.
...
Telechaind can now (optionally) asynchronously notify clients through a
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoinrpc>
        ...
        # optional bypass telechaind rpc basic auth
...
Restarting telechaind will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Telechain-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
...
- #6600 `ef4945f` Include telechain-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix telechain-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of telecli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override TELECHAIND and BITCOINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [telecli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add TELECHAIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
